...
public class ServerGameNetPortal : MonoBehaviour
{
    ...
    public void EndRound()
    {
        ...
        Scene scene = SceneManager.GetSceneByName("Playground");
    ...
    private void ApprovalCheck(NetworkManager.ConnectionApprovalRequest requst, NetworkManager.ConnectionApprovalResponse callback)
    {
        if (requst.Payload.Length > MaxConnectionPayload)
        {
        ...
        if (requst.ClientNetworkId == NetworkManager.Singleton.LocalClientId)
        {
            callback.CreatePlayerObject = false;
        ...
        string payload = Encoding.UTF8.GetString(requst.Payload);
        ...
        {
            clientSceneMap[requst.ClientNetworkId] = connectionPayload.clientScene;
            clientIdToGuid[requst.ClientNetworkId] = connectionPayload.clientGUID;
            clientData[connectionPayload.clientGUID] = new PlayerData(connectionPayload.playerName, requst.ClientNetworkId);
        }
        callback.CreatePlayerObject = false;
        callback.PlayerPrefabHash = 0;
        callback.Approved = true;
        ...
        gameNetPortal.ServerToClientConnectResult(requst.ClientNetworkId, gameReturnStatus);

        if (gameReturnStatus != ConnectStatus.Success)
        {
            StartCoroutine(WaitToDisconnectClient(requst.ClientNetworkId, gameReturnStatus));
        }
    }
    ...
    {
        string result = "";
        if (clientIdToGuid.TryGetValue(clientId, out var playerName))
        {
            if (clientData.TryGetValue(playerName, out var player))
            {
